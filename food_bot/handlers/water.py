from aiogram.filters import Command
from aiogram.types import Message
from aiogram import Router

from memory import ensure_user, ensure_day, today_key

router = Router()


@router.message(Command("log_water"))
async def log_water(message: Message):
    """
    ## /log_water

    Логирует выпитую воду за текущий день и показывает, сколько осталось до нормы.

    ### Формат команды
    `/log_water <мл>`

    Пример: `/log_water 250`

    ### Что делает обработчик
    1. Получает пользователя через `ensure_user`.
    2. Проверяет, что профиль настроен (есть `water_goal`), иначе просит `/set_profile`.
    3. Парсит количество миллилитров из сообщения и валидирует диапазон.
    4. Обновляет дневной лог воды: `day["water"] += ml`.
    5. Считает остаток до нормы с учётом добавки воды от тренировок (`burned_water`).
    6. Отправляет пользователю подтверждение и остаток.

    ### Валидация ввода
    - Значение должно быть целым числом.
    - Диапазон: `(0..5000]` (1–5000 мл).

    ### Используемые поля
    - `user["water_goal"]` — дневная цель воды (мл).
    - `user["burned_water"]` — добавка воды от тренировок за сегодня (мл).
    - `user["logged_water"]` — сколько воды выпито за день (мл).

    ### Ответ пользователю
    Пример:
    `💧 Записано: 250 мл.
     Осталось до нормы: 900 мл.`
    """
    user = ensure_user(message.from_user.id)
    if not user.get("water_goal"):
        return await message.answer("Сначала настройте профиль: /set_profile")

    parts = message.text.split(maxsplit=1)
    if len(parts) < 2:
        return await message.answer("Формат: /log_water <мл>  (например /log_water 250)")

    try:
        ml = int(parts[1])
        if ml <= 0 or ml > 5000:
            raise ValueError
    except Exception:
        return await message.answer("Введите количество мл числом, например /log_water 250")

    user["logged_water"] += ml

    day = ensure_day(user, today_key())
    day["water"] = user["logged_water"]

    left = max(0, user["water_goal"] - user["logged_water"] + user["burned_water"])
    await message.answer(f"💧 Записано: {ml} мл.\nОсталось до нормы: {left} мл.")
